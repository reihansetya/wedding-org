...
class Order(models.Model):
    ...
    tanggal_pengiriman = fields.Date(
        string='Tanggal Pengiriman', default=fields.Date.today())
    ...
    def _compute_total(self):
        for record in self:
            a = sum(self.env['wedding.orderpanggungdetail'].search(
                [('order_id', '=', record.id)]).mapped('harga'))
            b = sum(self.env['wedding.orderkursitamudetail'].search(
                [('orderk_id', '=', record.id)]).mapped('harga'))
            record.total = a + b
...
class OrderKursiTamuDetail(models.Model):
    ...
    orderk_id = fields.Many2one(
        comodel_name='wedding.order', string='Order Kursi')
    kursitamu_id = fields.Many2one(
        comodel_name='wedding.kursitamu', string='Kursi Tamu')
